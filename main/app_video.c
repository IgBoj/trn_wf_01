...
{
    ESP_LOGI(TAG, "Initializing esp_video with I2C handle: %p", i2c_bus_handle);
    
    esp_video_init_csi_config_t csi_config[] = {
        {
            .sccb_config = {
                .init_sccb = false,           // ← ИЗМЕНИТЬ НА true!
                .i2c_handle = i2c_bus_handle,
                .freq = 100000,              // ← Явно указать 100 кГц
            },
            .reset_pin = -1,
            .pwdn_pin = -1,
        },
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "esp_video_init FAILED: 0x%x (%s)", ret, esp_err_to_name(ret));
    } else {
        ESP_LOGI(TAG, "esp_video_init SUCCESS");
    }
...
int app_video_open(char *dev, video_fmt_t init_fmt)
{
    ...
    int fd = open(dev, O_RDWR); // Изменено на O_RDWR для надежности
    if (fd < 0) {
    ...
    // 1. ПРИНУДИТЕЛЬНО устанавливаем стандартное разрешение 640x480 и нужный формат
    struct v4l2_format target_format = {
        .type = type,
        .fmt.pix.width = 800,                  // <-- Явно задаем VGA
        .fmt.pix.height = 640,                 // <-- Явно задаем VGA
        .fmt.pix.pixelformat = init_fmt,       // <-- RGB565
        .fmt.pix.field = V4L2_FIELD_NONE,
    };

    if (ioctl(fd, VIDIOC_S_FMT, &target_format) == 0) {
        ESP_LOGI(TAG, "Successfully requested format: 800*640");
    } else {
        ESP_LOGW(TAG, "Failed to set 800*640, camera will use its default");
    }

    // 2. Читаем реальный формат, который в итоге установился в камере
    ...
    // 3. Сохраняем РЕАЛЬНЫЕ размеры для буферов и PPA
    ...
    ESP_LOGI(TAG, "ACTUAL camera format: width=%" PRIu32 " height=%" PRIu32 " fmt=0x%08x", 
             default_format.fmt.pix.width, 
             default_format.fmt.pix.height, 
             default_format.fmt.pix.pixelformat);

    // 4. Если формат все еще не совпадает с запрошенным, пытаемся изменить только pixelformat
    if (default_format.fmt.pix.pixelformat != init_fmt) {
        struct v4l2_format format_fix = {
            .type = type,
            .fmt.pix.width = default_format.fmt.pix.width,   // Оставляем то, что дала камера
            .fmt.pix.height = default_format.fmt.pix.height, // Оставляем то, что дала камера
            .fmt.pix.pixelformat = init_fmt,
            .fmt.pix.field = V4L2_FIELD_NONE,
        };
        if (ioctl(fd, VIDIOC_S_FMT, &format_fix) != 0) {
            ESP_LOGE(TAG, "failed to set pixelformat");
            goto exit_0;
        }
        // Перечитываем еще раз для уверенности
